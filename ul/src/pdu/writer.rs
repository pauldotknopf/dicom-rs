    ...
    W: Write,
{
    match pdu {
        PDU::AssociationRQ(data) => {
            write_pdu_association_rq(writer, data)?;
            Ok(())
        }
        PDU::AssociationAC(data) => {
            ...
            write_chunk_u32(writer, |writer| {
                ...
                // 0 is set.
                writer.write_u16::<BigEndian>(data.protocol_version)?;
                ...
                for presentation_context in &data.presentation_contexts {
                    write_pdu_variable_presentation_context_result(writer, presentation_context)?;
                }

                write_pdu_variable_user_variables(writer, &data.user_variables)?;

                Ok(())
            })
        }
        PDU::AssociationRJ(data) => {
            // 1 - PDU-type - 03H
        ...
        PDU::PData(data) => {
            ...
            write_chunk_u32(writer, |writer| {
                ...
                for presentation_data_value in &data.data {
                    write_chunk_u32(writer, |writer| {
...
pub fn write_pdu_association_rq<W>(writer: &mut W, data: &AssociateRQContainer) -> Result<()>
where
    W: Write,
{
    ...
    write_chunk_u32(writer, |writer| {
        ...
        let mut ae_title_bytes = data.called_ae_title.as_bytes().to_vec();
        ...
        // TODO: ensure ASCI/size
        let mut ae_title_bytes = data.calling_ae_title.as_bytes().to_vec();
        ae_title_bytes.resize(16, 32); // 32 is asci for space
        write_n(writer, &ae_title_bytes)?;
        ...
        write_pdu_variable_application_context_name(writer, &data.application_context_name)?;
